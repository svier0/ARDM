  ...
  methods: {
    ...
    execCommand() {
      ...
      if (hexStr.length > this.writeHexFileSize) {
        (window.__bunRpc && window.__bunRpc.getTempPath ? window.__bunRpc.getTempPath() : Promise.resolve('')).then((tmpDir) => {
          // target file name
          const fileName = `ardm_cv_${this.redisKey.toString('hex')}`;
          const filePath = tmpDir + '\\' + fileName;

          if (window.__bunRpc && window.__bunRpc.writeFile) {
            window.__bunRpc.writeFile(filePath, hexStr).then(() => {
            ...
            this.$message.error('File write not available');
          }
        });
      }
